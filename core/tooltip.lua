...
--		local inside_hook = false
        for tooltip in pairs(T.temp-T.set(GameTooltip, ItemRefTooltip)) do
...
function extend_tooltip(tooltip, link)
    ...
    if settings.merchant_buy then
        ...
        if price then
            tooltip:AddLine('Vendor Buy ' .. (limited and '(limited): ' or ': ') .. money.to_string2(price), aux.color.tooltip.merchant())
        end
    end
    if settings.merchant_sell then
        ...
        if price ~= 0 then
            ...
            (price and count and type(count)=="number" and count>1 and "x"..count.."   "..money.to_string2(price*count) or ""), aux.color.tooltip.merchant())
        end
    end
    ...
    if auctionable then
        if settings.value then
            tooltip:AddLine('Value: ' .. (value and money.to_string2(value) or UNKNOWN), aux.color.tooltip.value())
        end
        if settings.daily  then
            local market_value = history.market_value(item_key)
            tooltip:AddLine('Today: ' .. (market_value and money.to_string2(market_value) .. ' (' .. gui.percentage_historical(aux.round(market_value / value * 100)) .. ')' or UNKNOWN), aux.color.tooltip.value())
        end
    end
